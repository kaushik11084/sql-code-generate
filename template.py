...
def get_payloads(text, email_store, instruction, examples):
...
def get_payloads_example(counter, all_examples, instruction, email_store, href):
    emailDomain,user_id = get_user_email_and_id(email_store)
    sendTrackEvent(user_id, { "objectType": "nav-button", "text": "Edit example or instruction", "emailDomain": emailDomain}, None)
    example_payloads_output = []
    labels = ["Instruction", "Examples"]
    payloads = configs_dict['generate_btn_payload_files'].split(',')
    payload_file = payloads[0]

    # allow editing if user has logged in 
    ...
    for label, n in zip(labels, range(len(labels))):
        ...
        for i in range(len(examples)):
            ...
            children_of_table.append(dbc.Row([
                                                ...
                                                dbc.Col(dbc.Textarea(value=str(examples[i].split('Output:')[0][1:-1]) if examples != [] else "", disabled=True if(not t_login) else False, rows=4, class_name='carbon-input')),
                                                dbc.Col(dbc.Textarea(value=examples[i].split('Output:')[1][1:] if examples != [] else "", disabled=True if(not t_login) else False, rows=4, class_name='carbon-input'))
                                            ], style={'paddingTop':'10px'}))
        
        upload_button = dcc.Upload(id="upload-data", className="upload-data",
        children=[
            dbc.Button("Upload File", disabled=True if(not t_login) else False, outline=True, color="primary", n_clicks=0, className="carbon-btn"),
        ],
        max_size=50000,
        accept=".txt",
        disabled=True if(not t_login) else False
    )
        
        upload_file_note = dbc.Row(dbc.Col(
                            html.Div(
                                children=[html.I(className="bi bi-info-circle"),dcc.Markdown(configs_dict['error_msg_login_prompt'] if (not t_login) else "Allowed file types: .txt & File size limit to upload: 50Kb", style={"color": "#525252", "fontSize": "0.8rem","fontWeight": 400,"letterSpacing": 0,"paddingLeft":"0.5rem", "paddingTop":"3px"})],
                            style={"display":"flex"}),
                    )
                )
        
        instrction_structure = dbc.Row([
                dbc.Col(children=[dbc.Row(dbc.Textarea(id='sample-instruction', value=instruction, disabled=True if(not t_login) else False, rows=18, class_name='carbon-input', style={"margin":"1rem 4rem 0 0", "resize":"auto"})),
        ...
        if(label=="Instruction"):
            ...
            dbc.Tab([
                        ...
                        html.Div(children=[
                                            dbc.Button('Add example', id='add-row-button-{}'.format(payload_file), disabled=True if(not t_login) else False, n_clicks=0, className="carbon-btn" ,outline=True, color="primary"),
                                            dcc.Upload(id="upload-example-{}".format(payload_file), className="upload-data",
                                                                    children=[
                                                                        dbc.Button("Upload examples", outline=True, disabled=True if(not t_login) else False, color="primary", n_clicks=0, className="carbon-btn"),
                        ...
                        dbc.Row(upload_file_note, style={"paddingLeft":"8.3%", "paddingTop": "1rem", "visibility": "visible" if(not t_login) else "hidden"}),
                        html.Div(children=[
...
def codegen_fn(text, codegen_payload_json, type):
    ...
    print(type)
    answer = "<html><pre>" + response_llm_json['results'][0]['generated_text'] + "</pre></html>"
...
def generate_output_llm(n, verification_completed, s, text, examples, instruction, href: str, email_store, is_open):
    if(n>0 or verification_completed==True or s>0):
        isCustomText = "No"
        flg=1

        current_example = examples
        actual_example = given_example

        current_example_edited = current_example.replace("\n","")
        actual_example_edited = actual_example.replace("\n","")
        if(current_example_edited!=actual_example_edited or actual_instruction!=instruction):
            flg=0
        # Input is tampered
        if(text not in suggestions_list or flg==0):
            if(text.strip()==""):
                time.sleep(0.5)
...
def generate_output_llm_sp(n, s, text, verification_completed):
    if(n>0 or verification_completed==True or s>0):
...
def toggle_payload_modal(n1, is_open, text, instruction, examples):
    if n1:
        op=[]
        if(not is_open):
            op=get_payloads(text, None, instruction, examples)
...
def toggle_example_modal(n1, is_open, counter, examples, instruction, email_store, href: str):
    if n1:
        op=[]
        if(not is_open):
            op=get_payloads_example(counter, examples, instruction, email_store, href)
